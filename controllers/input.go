...
func SaveInput(args map[string]any) models.Response {
	...
	decoded, err := utils.AnyInputBodyEncodedToBytes(argsSaveInput)
	if err != nil {
		return models.Error400Response(err)
	}
	// Subir a S3
...
func ExplodeInput(args map[string]any) models.Response {
	argsExplodeInput, err := models.ExplodeInputFileArgs(args)
	if err != nil {
		return models.Error400Response(err)
	}
	data, err := providers.GetMainInputFileFromS3(argsExplodeInput)
	if err != nil {
		return models.Error400Response(err)
	}
	...
	prefixExplodedS3 := utils.GetPrefixExplodedInputs(argsExplodeInput)
	quantityFiles, err := providers.UploadJSONFilesConcurrentlyV1(explodedRows, prefixExplodedS3)
	if err != nil {
		return models.Error400Response(err)
	}
	return models.Response{
		StatusCode: 200,
		Body: models.MetaDataExplodedInput{
			TaskID:         argsExplodeInput.Id,
			TaskName:       argsExplodeInput.TaskName,
			PrefixExploded: prefixExplodedS3,
			Format:         "json",
			UploadedAt:     0,
